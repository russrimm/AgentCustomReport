...
flowchart TD
    ...
    MergeData --> CheckReports{Reports<br/>Directory Exists?}
    CheckReports -->|No| CreateDir[Create Reports Directory]
    CheckReports -->|Yes| ExportCSV[Export to CSV<br/>Reports/CopilotAgents_CompleteReport_*.csv]
    CreateDir --> ExportCSV
...
**Output:** `Reports/CopilotAgents_CompleteReport_YYYYMMDD_HHMMSS.csv` with comprehensive agent data
- Automatically creates Reports directory if it doesn't exist
- All reports saved to scripts/Reports/ for organization
